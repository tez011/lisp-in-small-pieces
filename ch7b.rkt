...
; The virtual machine.
(define CONSTANT (lambda args (list (cons 'CONSTANT args))))
(define SHALLOW-ARGUMENT-REF (lambda args (list (cons 'SHALLOW-ARGUMENT-REF args))))
(define DEEP-ARGUMENT-REF (lambda args (list (cons 'DEEP-ARGUMENT-REF args))))
(define GLOBAL-REF (lambda args (list (cons 'GLOBAL-REF args))))
(define CHECKED-GLOBAL-REF (lambda args (list (cons 'CHECKED-GLOBAL-REF args))))
(define PREDEFINED (lambda args (list (cons 'PREDEFINED args))))
(define JUMP-FALSE (lambda args (list (cons 'JUMP-FALSE args))))
(define GOTO (lambda args (list (cons 'GOTO args))))
(define SET-SHALLOW-ARGUMENT! (lambda args (list (cons 'SET-SHALLOW-ARGUMENT! args))))
(define SET-DEEP-ARGUMENT! (lambda args (list (cons 'SET-DEEP-ARGUMENT! args))))
(define SET-GLOBAL! (lambda args (list (cons 'SET-GLOBAL! args))))
(define ARITY=? (lambda args (list (cons 'ARITY=? args))))
(define ARITY>=? (lambda args (list (cons 'ARITY>=? args))))
(define CREATE-CLOSURE (lambda args (list (cons 'CREATE-CLOSURE args))))
(define PACK-FRAME! (lambda args (list (cons 'PACK-FRAME! args))))
(define PUSH-VALUE (lambda args (list (cons 'PUSH-VALUE args))))
(define POP-FUNCTION (lambda args (list (cons 'POP-FUNCTION args))))
(define PRESERVE-ENV (lambda args (list (cons 'PRESERVE-ENV args))))
(define FUNCTION-INVOKE (lambda args (list (cons 'FUNCTION-INVOKE args))))
(define RESTORE-ENV (lambda args (list (cons 'RESTORE-ENV args))))
(define POP-FRAME! (lambda args (list (cons 'POP-FRAME! args))))
(define ALLOCATE-FRAME (lambda args (list (cons 'ALLOCATE-FRAME args))))
(define EXTEND-ENV (lambda args (list (cons 'EXTEND-ENV args))))
(define UNLINK-ENV (lambda args (list (cons 'UNLINK-ENV args))))
(define POP-CONS-FRAME! (lambda args (list (cons 'POP-CONS-FRAME! args))))
(define ALLOCATE-DOTTED-FRAME (lambda args (list (cons 'ALLOCATE-DOTTED-FRAME args))))
(define CALL0 (lambda args (list (cons 'CALL0 args))))
(define POP-ARG1 (lambda args (list (cons 'POP-ARG1 args))))
(define POP-ARG2 (lambda args (list (cons 'POP-ARG2 args))))
(define INVOKE1 (lambda args (list (cons 'INVOKE1 args))))
(define INVOKE2 (lambda args (list (cons 'INVOKE2 args))))
(define INVOKE3 (lambda args (list (cons 'INVOKE3 args))))
(define RETURN (lambda args (list (cons 'RETURN args))))
(define FINISH (lambda args (list (cons 'FINISH args))))
...
(define (LiSP7b)
  (define (toplevel)
    ...
    (set! *arg2* #f)
    (set! *stack-index* 0)
    (pretty-print (meaning e r.init #t))
    (newline)
    (toplevel))
  (toplevel))
