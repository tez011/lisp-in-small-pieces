...
(define (match* p e literals sk fk)
  (cond
    ((and (pair? p) (pair? (cdr p)) (eq? '... (cadr p)))
     (let loop ((e e)
                (b '()))
       (if (null? e)
           (match* (cddr p) e literals
             (lambda (b^) (sk `((... . ,b) . ,b^)))
             fk)
           (match* (car p) (car e) literals
             (lambda (b^)
               (loop (cdr e) (append b (list b^))))
             (lambda ()
               (match* (cddr p) e literals
                 (lambda (b^) (sk `((... . ,b) . ,b^)))
                 fk))))))
    ((and (pair? p) (pair? e))
     (match* (car p) (car e) literals
       (lambda (b)
         (match* (cdr p) (cdr e) literals
           (lambda (b^) (sk (append b b^)))
           fk))
       fk))
    ((or (eq? p '_))
     (sk '()))
    ((and (member p literals) (eq? p e))
     (sk '()))
    ((symbol? p)
     (sk (list (cons p e))))
    ((and (null? p) (null? e))
...
(check-expect (match* '(let ((x e) ...) b) '(let ((x 5) (y 6)) (+ x y)) '(let) (lambda (b) b) (lambda () #f)) '((... ((x . x) (e . 5)) ((x . y) (e . 6))) (b . (+ x y))))
...
(check-expect (match* '(let ((name val) ...) body1 body2 ...) '(let ((x 5) (y 6)) (set! a 4) (+ x y)) '(let)
