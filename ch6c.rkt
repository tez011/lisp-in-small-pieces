...
; A compiler that emits callable thunks, which display instructions to a virtual machine.
...
; The virtual machine. This block, aside from the interpreter itself, is the only difference between this and ch6b.
(define CONSTANT (lambda args (cons 'CONSTANT args)))
(define SHALLOW-ARGUMENT-REF (lambda args (cons 'SHALLOW-ARGUMENT-REF args)))
(define DEEP-ARGUMENT-REF (lambda args (cons 'DEEP-ARGUMENT-REF args)))
(define CHECKED-GLOBAL-REF (lambda args (cons 'CHECKED-GLOBAL-REF args)))
(define PREDEFINED (lambda args (cons 'PREDEFINED args)))
(define ALTERNATIVE (lambda args (cons 'ALTERNATIVE args)))
(define SHALLOW-ARGUMENT-SET! (lambda args (cons 'SHALLOW-ARGUMENT-SET! args)))
(define DEEP-ARGUMENT-SET! (lambda args (cons 'DEEP-ARGUMENT-SET! args)))
(define GLOBAL-SET! (lambda args (cons 'GLOBAL-SET! args)))
(define SEQUENCE (lambda args (cons 'SEQUENCE args)))
(define FIX-CLOSURE (lambda args (cons 'FIX-CLOSURE args)))
(define NARY-CLOSURE (lambda args (cons 'NARY-CLOSURE args)))
(define TR-REGULAR-CALL (lambda args (cons 'TR-REGULAR-CALL args)))
(define REGULAR-CALL (lambda args (cons 'REGULAR-CALL args)))
(define STORE-ARGUMENT (lambda args (cons 'STORE-ARGUMENT args)))
(define ALLOCATE-FRAME (lambda args (cons 'ALLOCATE-FRAME args)))
(define FIX-LET (lambda args (cons 'FIX-LET args)))
(define TR-FIX-LET (lambda args (cons 'TR-FIX-LET args)))
(define CONS-ARGUMENT (lambda args (cons 'CONS-ARGUMENT args)))
(define ALLOCATE-DOTTED-FRAME (lambda args (cons 'ALLOCATE-DOTTED-FRAME args)))
(define CALL0 (lambda args (cons 'CALL0 args)))
(define CALL1 (lambda args (cons 'CALL1 args)))
(define CALL2 (lambda args (cons 'CALL2 args)))
(define CALL3 (lambda args (cons 'CALL3 args)))
...
(define (LiSP6c)
  (define (toplevel)
    (set! *env* sr.init)
    (pretty-print (meaning (read) r.init #t))
    (newline)
    (toplevel))
  (toplevel))
